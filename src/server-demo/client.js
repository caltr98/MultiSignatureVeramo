...
async function main() {
    ...
    // ---- setup actors ----
    const issuer = await postJson(baseUrl, '/setup', { name: 'issuer' });
    const holders = [];
    for (let i = 0; i < holdersCount; i++) {
        holders.push(await postJson(baseUrl, '/setup', { name: `holder-${i}` }));
    }
    // ---- create a VC (JWT) ----
    const vcResp = await postJson(baseUrl, '/vc/create', {
    ...
    const aggResp = await postJson(baseUrl, '/bls/aggregate', {
        keys: holders.map((h) => h.bls_pub_key),
    });
    const aggregatedKey = aggResp.aggregatedKey;
    // ---- create VP payload ----
    const vpPayloadResp = await postJson(baseUrl, '/vp/generatepayload', {
    ...
    for (const h of holders) {
        const sigResp = await postJson(baseUrl, '/vp/signbls', {
        ...
        if (!payloadToSign)
            payloadToSign = sigResp.payloadToSign;
    }
    if (!payloadToSign)
        throw new Error('missing payloadToSign from /vp/signbls');
    // ---- collect PoOs ----
    const proofsOfOwnership = [];
    for (const h of holders) {
        const pooResp = await postJson(baseUrl, '/vp/poo', {
            did: h.did,
            kid_eth: h.kid_eth,
            payloadToSign,
        });
    ...
    // ---- create full PoO VP ----
    const fullVpResp = await postJson(baseUrl, '/vp/createfullvp', {
    ...
    // ---- verify ----
    const verifyResp = await postJson(baseUrl, '/vp/multi/verify', { usePoO: true, vp });
