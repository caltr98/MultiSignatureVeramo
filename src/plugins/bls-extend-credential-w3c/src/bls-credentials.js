 ...
 */
export async function verifyCredentialProofOfOwnershipMultiSignatureBls(credential, context, resolutionOptions) {
    ...
    try {
        ...
        const resolvedVMs = await Promise.all(methods.map(async (method) => {
...
// NEW: aggregate OF MULTI-SIGNATURE to make Multi Signature Verifiable Credential
export async function generateProofOfOwnershipMultiIssuerVerifiableCredentialBls(credential, proofs_of_ownership, list_of_signatures, settings) {
 ...
 */
export async function verifyCredentialBls(credential, context, resolutionOptions) {
    const proof = credential.proof;
 ...
 * Verify a BLS-signed Verifiable Credential
 */
export async function verifyCredentialMultiSignatureBls(credential, context, resolutionOptions) {
    const proof = credential.proof;
    if (!proof || !proof.type || !proof.signatureValue) {
        return {
            verified: false,
