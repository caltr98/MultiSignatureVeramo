...
import {
    ...
    IKey,
    IService, TKeyType, IKeyManagerCreateArgs, MinimalImportableKey,
} from '@veramo/core-types'
import { schema } from '@veramo/core-types'
import { AbstractDIDStore } from './abstract-identifier-store.js'

export type IDIDBlsManagerAddKeyArgs= IDIDManagerAddKeyArgs & {
    NoPublish?: boolean
}
 ...
 * @public
 */
export class DIDManagerBls implements IAgentPlugin {
  /**
   * Plugin methods
   * @public
   */
  ...
  async didManagerAddKey(
    { did, key, options,NoPublish }: IDIDBlsManagerAddKeyArgs,
    context: IAgentContext<IKeyManager>,
  ): Promise<any> {
    const identifier = await this.store.getDID({ did })
    const provider = this.getProvider(identifier.provider)
    ...
    identifier.keys.push(key)
    const result = await this.store.importDID(identifier)
    return result
  }
