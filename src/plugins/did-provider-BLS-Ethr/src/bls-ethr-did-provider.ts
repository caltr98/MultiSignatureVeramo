//NEW:Extended with ADDKEY allowing to publish BLS KEY of chainsafe/bls
import {
    IAgentContext,
    IIdentifier,
    IKey,
    IKeyManager, IKeyManagerCreateArgs,
    IService,
...
//TODO: PROBLEM Needed to add manually as import failed!
 ...
 * @public
 */
export class BlsEthrDIDProvider extends AbstractIdentifierProvider {
    ...
    async addKey(
        ...
        if (options?.metaIdentifierKeyId) {
            ...
            const canonicalSignature = await BlsEthrDIDProvider.createMetaSignature(context, identifier, metaHash)
    ...
    async removeKey(
        ...
        if (args.options?.metaIdentifierKeyId) {
            ...
            const canonicalSignature = await BlsEthrDIDProvider.createMetaSignature(context, args.identifier, metaHash)
