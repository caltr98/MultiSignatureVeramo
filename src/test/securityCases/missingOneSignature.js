...
async function missOneSignatureVCAggregateKeysToSignatures(issuers, holder, claimCount, valueSize, seed = 42) {
    ...
    let signaturesHex = signaturesHexAndSignatures.signatures;
    let signaturesPayloadSigned = signaturesHexAndSignatures.payloads;
    //ADDITION: REMOVE THE LAST SIGNATURE
    signaturesHex.pop();
    signaturesPayloadSigned.pop();
    ...
    console.log("removing last signature result" + JSON.stringify(result));
    return vcFull;
}
export { missOneSignatureVCAggregateKeysToSignatures };
