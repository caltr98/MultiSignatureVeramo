...
async function benchmarkStep<T>(
    label: string,
...
for (let i = 0; i < RUNS; i++) {
    ...
    for (const issuer of issuers) {
        const payload = await generateVCPayload(
            holder.did,
            claims_n,
            claims_size,
            42
    ...
    const vp = await benchmarkStep('Create VP (N VCs)', timings, () =>
        createPresentation(credentials, holder.did)
    );

    await benchmarkStep('Verify VP (N VCs)', timings, () =>
        verifyVP(vp as VerifiablePresentation)
    );
