...
} from '../../src/plugins/bls-extend-credential-w3c/src/bls-credentials.js'
import { agent } from '../../src/veramo/setup.js'
import { bls12_381 as bls } from '@noble/curves/bls12-381'
import { bytesToHex } from '@noble/hashes/utils'

type AgentInfo = { did: string; kid_bls: string; bls_pub_key: string }
const strip0x = (hex: string) => (hex.startsWith('0x') ? hex.slice(2) : hex)
const pointFromHex = (hex: string) => bls.G1.ProjectivePoint.fromHex(strip0x(hex))
const pointToHex = (p: any) => '0x' + bytesToHex(p.toRawBytes(true))
...
async function getEthKeyKidForDid(did: string): Promise<string> {
  ...
  const ethKey = identifier.keys.find(
    (k: any) => k.type === 'Secp256k1' || k.meta?.alg === 'eth_signMessage',
  )
  if (!ethKey) throw new Error(`No Secp256k1 key found for DID ${did}`)
  return ethKey.kid
}

async function buildLegitCredential(
  issuers: AgentInfo[],
  holderDid: string,
  blsBackend?: 'chainsafe' | 'noble',
) {
  ...
  const vc = await generateProofOfOwnershipMultiIssuerVerifiableCredentialBls(
    payload,
    proofsOfOwnership,
  ...
  const verification = await verifyCredentialProofOfOwnershipMultiSignatureBls(
    vc as any,
    { agent } as any,
    undefined,
    blsBackend,
...
async function attemptRogueKeyAttack(
  honest: AgentInfo,
  malicious: AgentInfo[],
  holderDid: string,
  blsBackend?: 'chainsafe' | 'noble',
  ...
  const rogueVc = await generateProofOfOwnershipMultiIssuerVerifiableCredentialBls(
    payload,
    forgedPoOs,
    ['0x' + rogueSignature] as string[],
    undefined,
    blsBackend,
  )

  const verification = await verifyCredentialProofOfOwnershipMultiSignatureBls(rogueVc as any, { agent } as any, undefined, blsBackend)

  return { rogueVc, verification }
}

async function main() {
  const blsBackend = (process.env.VERAMO_BLS_BACKEND as 'chainsafe' | 'noble' | undefined) ?? undefined
